...
import { ExpenseData, IncomeData, PieChartData } from "@/types/chart";
import { TransactionType } from "@/types/transaction";

const transformToPieData = (transactions: TransactionType[], selectedMonth: string): PieChartData => {
  ...
  const incomeData: IncomeData = filteredTransactions
    ...
    const expenseData: ExpenseData = filteredTransactions.filter((t) => t.type === "Expense").reduce((acc: ExpenseData, t) => {
...
export default transformToPieData;
